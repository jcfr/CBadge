...
router.get('/:project/:action/:owner/:repo/:tag', function(req, res) {
  ...
  get(options, res, function (commit) {
    if(!commit.sha){
        res.send(500);
        return;
      }
...
router.get('/:project/pullRequest/:owner/:repo/:number/:sha', function(req, res) {
  ...
  get(options, res, function (pullRequest) {
...
router.get('/:project/:revision/coverage.svg', function(req, res) {
  ...
  var query = 'http://open.cdash.org/api/?method=build&task=revisionstatus&project='+req.params.project+'&revision='+req.params.revision
  get(query, res, function (coverage) {
    var loctested = 0;
    var locuntested = 0;
